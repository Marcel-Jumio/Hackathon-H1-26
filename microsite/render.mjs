 ...
 * @param {object} [sdkAssets] - paths to the @jumio/websdk runtime, relative to the output page
 * @param {string} [sdkAssets.script] - index.js (the SDK entry, type=module)
 * @param {string} [sdkAssets.modulePreload] - the hashed chunk referenced by index.js
...
<head>
  ...
  <!-- @jumio/websdk runtime (npm package, vendored locally — see microsite/build.mjs) -->
  <script type="module" crossorigin src="${escapeHtml(sdkAssets.script ?? '../../vendor/websdk/index.js')}"></script>
  <link rel="modulepreload" crossorigin href="${escapeHtml(sdkAssets.modulePreload ?? '../../vendor/websdk/js/index.js')}" />
...
${steps
    .map((label, i) => {
      ...
      <jumio-sdk dc="${escapeHtml(session.dc ?? 'us')}" token="${escapeHtml(session.token ?? 'REPLACE_WITH_SESSION_TOKEN')}"></jumio-sdk>`;
